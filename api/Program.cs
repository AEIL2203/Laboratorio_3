...
app.MapPost("/api/auth/register", async (CreateUserPublicDto reg, IPasswordHasher hasher, IJwtService jwt) =>
{
    ...
    var exists = await c.ExecuteScalarAsync<bool>(
        "SELECT 1 FROM HoopsDB.core.Users WHERE UserName = @UserName",
        new { reg.UserName });
    ...
    var role = !string.IsNullOrWhiteSpace(reg.Role) && reg.Role.ToLower() == "admin" ? "Admin" : "User";
    ...
    var userId = await c.ExecuteScalarAsync<int>(
        "INSERT INTO HoopsDB.core.Users (UserName, PasswordHash, Role, Email, Active) " +
        "VALUES (@UserName, @PasswordHash, @Role, @Email, 1); " +
    ...
    var token = jwt.CreateToken(reg.UserName, role, null);
